...
module.exports = function ( grunt ) {
  ...
  var config = {
    ...
    watch: {
      ...
      scripts: {
        files: [ '**/global.js', '**/variation.js' ],
        options: {
          livereload: livereloadOptions
        }
      },
      conditional: {
        files: [ '**/conditional.js' ],
        tasks: [ 'compileConditional' ]
      },
      light: {
        files: [ '**/*.js', '**/*.css' ],
        options: {
  ...
  grunt.registerTask( 'compileConditional', function () {
    //These are the characters which will be replaced
    var entityMap = {
      "&": '&amp;',
      '"': '\\\"',
    ...
    // Identify all project JS files with a glob
    var projectArray = glob.sync( '**/conditional.js' );

    projectArray.forEach( function ( conditionalJS ) {
      var directory = path.dirname(conditionalJS);
      try {
        var experimentDetails = JSON.parse(fs.readFileSync(directory + '/experiment.json'));
        experimentDetails.conditional_code = String(fs.readFileSync(conditionalJS));
        fs.writeFileSync(directory + '/experiment.json', JSON.stringify(experimentDetails, null, 2));
      } catch (error) {
        return console.log(conditionalJS, ' failed: ', error);
      }
    } )
  } );
};
